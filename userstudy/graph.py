...
def graph_anova(results):
    ...
    # Performing ANOVA
    df['View'] = df['Condition'].apply(lambda x: 'TRACKER' if 'TRACKER' in x else 'STATIC')
    df['Offset'] = df['Condition'].apply(lambda x: 'OFFSET' if 'OFFSET' in x else 'NOT_OFFSET')

	# Print the means for each condition
    print(df.groupby(['View', 'Offset'])['Time'].mean())
    
    # Perform the ANOVA
    model = ols('Time ~ C(View) * C(Offset)', data=df).fit()
    anova_table = sm.stats.anova_lm(model, typ=2)
    print(anova_table)

    # Extract p-values
    p_type = anova_table.loc['C(View)', 'PR(>F)']
    p_offset = anova_table.loc['C(Offset)', 'PR(>F)']

    # Generate significance stars
    stars_type = significance_stars(p_type)
    stars_offset = significance_stars(p_offset)

    # Plotting interaction plot
    plt.figure(figsize=(10, 10))
    ax = sns.pointplot(data=df, x='Offset', y='Time', hue='View', dodge=True, markers=['o', 's'], capsize=.1, errwidth=1, palette=['#ff7c37ff', '#0073c0ff', '#2aaa00ff'], linestyles=['-', '--'])
    plt.xlabel('Offset')
    plt.ylabel('Time (ms)')
    plt.ylim(1800, 3000)
    ...
    # Adding significance stars and lines
    ymax = df['Time'].max() * 0.125
    yoffset = ymax * 0.01  # Offset for positioning stars above the lines

    # Adjust the positions of the stars as needed
    plt.plot([0, 1], [ymax-3*yoffset, ymax-3*yoffset], color='red')
    plt.text(0.5, ymax - 2.75*yoffset, stars_type, ha='center', va='bottom', color='black', size=30)
    plt.plot([-0.1, 0.1], [ymax - 7*yoffset, ymax - 7*yoffset], color='black')
    plt.text(0, ymax - 6.75*yoffset, stars_offset, ha='center', va='bottom', color='black', size=30)
...
def graph_preferences(data_list):
    ...
    for entry in data_list:
        ## Don't fix this the spelling error is important
    ...
    rank_colors = plt.cm.get_cmap('tab20c', n_ranks).colors
    ...
    ranks_data = []
    for prefs in preference_list:
        ranks = [prefs.index(condition + "\xa0") + 1 for condition in unique_conditions]
        ranks_data.append(ranks)
    
    ranks_data = np.array(ranks_data).T  # Transpose to have conditions as rows

    print(ranks_data)

    # Perform Friedman test
    test_stat, p_value = friedmanchisquare(*ranks_data)
    print(f'Friedman test statistic: {test_stat}, p-value: {p_value}')

    if p_value < 0.05:
        print("There is a statistically significant difference between the conditions.")
    else:
        print("No statistically significant difference between the conditions.")
...
def graph_distance_per_task(distance_results):
    distance_cutoff = 30
    ...
    for user_id, challenges in distance_results.items():
        for challenge_num, conditions in challenges.items():
            if challenge_num not in distances_by_challenge:
                distances_by_challenge[challenge_num] = {
                    'TRACKER': [],
                    'TRACKER_OFFSET': [],
                    'STATIC': [],
                    'STATIC_OFFSET': []
            ...
            for condition, segments in conditions.items():
                for i in range(1, len(segments)):
                    distances_by_challenge[challenge_num][condition] += get_distances_for_segment(segments, total_failed_segments, user_id, challenge_num, condition, i)

    for challenge_num, conditions in distances_by_challenge.items():
        print(f"Challenge {challenge_num}:")
        for condition, distances in conditions.items():
            print(f"{condition}: {len(distances)}")
        ...
        plt.xlabel('Distance')
        plt.ylabel('Density')
        plt.title(f'Distribution of Distances for Challenge {challenge_num}')
        plt.grid(True, linestyle='--', linewidth=0.7)
        plt.xlim(0, distance_cutoff)
        plt.savefig(f'misc/graphs/distance-distribution-challenge-{challenge_num}.pdf', transparent=True, bbox_inches='tight')
        plt.show()

# data refrence
# {id :{ challenge_num: {condition: [[[(x,y,z),(x,y,z),....]],[...],...]}}}
# def graph_direction(distance_results):
#     # Update LaTeX style settings
#     plt.rcParams.update({
#         "text.usetex": True,
#         "font.family": "serif",
#         "font.serif": ["Computer Modern Roman"],
...
#     collapsed_results = {
#         "TRACKER": {"x": [], "y": [], "z": []},
#         "TRACKER_OFFSET": {"x": [], "y": [], "z": []},
#         "STATIC": {"x": [], "y": [], "z": []},
#         "STATIC_OFFSET": {"x": [], "y": [], "z": []}
#     }

#     for challenge_id, challenges in distance_results.items():
#         for challenge_num, conditions in challenges.items():
#             for condition, segments in conditions.items():
#                 for segment_idx, segment in enumerate(segments):
#                     x_values = [vec[1][0] for vec in segment]
#                     y_values = [vec[1][1] for vec in segment]
#                     z_values = [vec[1][2] for vec in segment]

#                     std_x = np.std(x_values)
#                     std_y = np.std(y_values)
#                     std_z = np.std(z_values)

#                     collapsed_results[condition]["x"].append(std_x)
#                     collapsed_results[condition]["y"].append(std_y)
#                     collapsed_results[condition]["z"].append(std_z)

#                     results.append({
#                         'Challenge ID': challenge_id,
#                         'Challenge Num': challenge_num,
#                         'Condition': condition,
#                         'Segment': segment_idx,
#                         'Std X': std_x,
#                         'Std Y': std_y,
...
#     # Prepare data for plotting
#     conditions = ["TRACKER", "STATIC", "TRACKER_OFFSET", "STATIC_OFFSET"]
#     directions = ["x", "y", "z"]
#     combined_data = []
...
#                 'Condition': condition,
#                 'Direction': direction,
#                 'Std Dev': np.mean(collapsed_results[condition][direction])
#             })

#     combined_df = pd.DataFrame(combined_data)

#     # Define colors for consistency
#     colors = ['#8f45a3ff', '#0073c0ff', '#ff7c37ff', '#2aaa00ff']
#     bar_width = 0.2

#     fig, ax = plt.subplots()

#     for i, direction in enumerate(directions):
#         df_plot = combined_df[combined_df['Direction'] == direction]
#         ax.bar(
#             np.arange(len(conditions)) + (i * bar_width) - bar_width,
#             df_plot['Std Dev'],
#             width=bar_width,
#             label=f'Std Dev in {direction.upper()}',
#             color=colors[i]
#         )
...
#     ax.set_ylabel('Mean Standard Deviation (cm)')
#     # ax.set_title('Mean Standard Deviation by Condition and Direction')
#     ax.legend()

#     plt.grid(True, linestyle='--', linewidth=0.7)
#     plt.tight_layout()

#     plt.savefig('misc/graphs/direction-distribution.pdf', transparent=True, bbox_inches='tight')
#     plt.show()
    
# Set pandas display option to show more decimal places
pd.set_option('display.float_format', lambda x: f'{x:.8f}')  # Adjust the precision as needed

# Your function with an additional line to set precision for the print output
def graph_direction(distance_results):
    results = []
    collapsed_results = {
        "TRACKER": {"x": [], "y": [], "z": []},
    ...
    conditions = list(collapsed_results.keys())
    directions = ["x", "y", "z"]
    test_results = {}

    for direction in directions:
        direction_data = {
            condition: collapsed_results[condition][direction]
            for condition in conditions
        }

        df_direction = pd.DataFrame({
            'Value': sum(direction_data.values(), []),
            'Condition': sum([[cond] * len(direction_data[cond]) for cond in conditions], [])
        })

        df_direction['View'] = df_direction['Condition'].apply(lambda x: 'TRACKER' if 'TRACKER' in x else 'STATIC')
        df_direction['Offset'] = df_direction['Condition'].apply(lambda x: 'OFFSET' if 'OFFSET' in x else 'NOT_OFFSET')

        model = ols('Value ~ C(View) * C(Offset)', data=df_direction).fit()
        anova_table = sm.stats.anova_lm(model, typ=2)

        p_view = anova_table.loc['C(View)', 'PR(>F)']
        p_offset = anova_table.loc['C(Offset)', 'PR(>F)']
        p_interaction = anova_table.loc['C(View):C(Offset)', 'PR(>F)']
        print(direction)
        print(anova_table)
        test_results[direction] = {
            'p_view': p_view,
            'p_offset': p_offset,
